...
int MainWindow::getASKAValues()
{

    QString aux;
    uint32_t ic_add, amp, freq, ontime, offtime, ramp, phase;
    uint32_t ele1, ele2, enable;

    aux = ui->lineEditAskaICaddr->text();
    ic_add = aux.toInt();

    if (ic_add > 3) {
        qDebug()<<"Error getASKAValues: Wrong IC address";
        return 1;
    }

    uint8_t ASIC_addr;
    ASIC_addr = m_ASKA.getASKA_ICaddress(ic_add);

    aux = ui->lineEditAskaAmp->text();
    amp = aux.toInt();

    aux = ui->lineEditAskaFreq->text();
    freq = aux.toInt();

    aux = ui->lineEditAskaOnTime->text();
    ontime = aux.toInt();

    aux = ui->lineEditAskaOffTime->text();
    offtime = aux.toInt();

    aux = ui->lineEditAskaRamp->text();
    ramp = aux.toInt();

    aux = ui->lineEditAskaPhase->text();
    phase = aux.toInt();

    aux = ui->lineEditAskaEle1->text();
    ele1 = aux.toInt();

    aux = ui->lineEditAskaEle2->text();
    ele2 = aux.toInt();

    enable =ui->checkBoxAskaEnable->isChecked();

    if ((ele1 == ele2) && (ele1 || ele2)) {
        qDebug()<<"Error getASKAValues: Ele1 must be different than Ele2 during a program (exception Ele1 = Ele2 = 0). ASKA not updated!";
        return 1;
    }

    // Data valid
    m_ASKA.setProgram(ASIC_addr,freq,amp,ontime,offtime,ramp,enable,phase,ele1,ele2);
...
void MainWindow::on_pushButtonAskaProgram_clicked()
{
    int err;
    ...
    if (err) {
        qDebug()<<"Failed getASKAValues";
    } else {
        QByteArray data;

        data = m_ASKA.getCommand();
